...
<body>
    ...
    <div class="center-paragraph">
        <h4>KWITANSI PEMBAYARAN ALUMNI</h4>
    </div>
    <table style="width: 100%; font-size: 10pt; margin-top: 10px;">
        <tbody>
            <tr>
                <td>Nama : {{ $pembayaran->alumni->nama }}</td>
            </tr>
            <tr>
                <td>Jenjang : {{ strtoupper($pembayaran->alumni->jenjang) }}</td>
                <td style="text-align: right;">Tanggal Pembayaran : {{ date('d F Y', strtotime($pembayaran->tanggal_pembayaran)) }}</td>
            </tr>
        </tbody>
    </table>
    <div class="mt-2"></div>
    <table class="tb-tagihan">
        ...
        <tbody>
            <tr>
                <td> {{ $pembayaran->alumni->jenis_keuangan }}</td>
...
<div class="mt-2"></div>
    ...
    <p style="font-style: italic;">Terbilang: {{ \App\Helpers\Terbilang::make((int) $pembayaran->jumlah_dibayar) }}</p>
    <div class="qrcode">
