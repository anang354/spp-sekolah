    ...
    <div class="max-w-md text-sm lg:text-base w-full bg-green-50 rounded-lg text-green-700 inset-ring inset-ring-green-600/20 p-6">
        <div class="w-full flex flex-row mb-6 gap-4">
            <div>
                ...
                <p class="mb-2">Nama</p>
                <p class="mb-2">Jenjang</p>
                <p class="mb-2">Nominal</p>
            </div>
            <div class="font-medium">
                <p class="mb-2">{{ $pembayaran->tanggal_pembayaran }}</p>
                <p class="mb-2">{{ $pembayaran->alumni->jenis_keuangan }}</p>
                <p class="mb-2">{{ $pembayaran->alumni->nama }}</p>
                <p class="mb-2">{{ $pembayaran->alumni->jenjang }}</p>
                <p class="mb-2">Rp {{ number_format($pembayaran->jumlah_dibayar, 0, ',', '.') }}</p>
            </div>
        </div>
